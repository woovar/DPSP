...
class JSON_API_Post {
  ...
  function JSON_API_Post($wp_post = null, $sidebar=null) {
    if (!empty($wp_post)) {
//     	var_dump('sidebar=');
//     	var_dump($sidebar);
    	if(empty($sidebar)){
    		...
    		$this->import_wp_object_for_sidebar($wp_post);
    	}
    }
  }
  ...
  function import_wp_object_for_sidebar($wp_post) {
  	global $json_api, $post;
//   	$date_format = $json_api->query->date_format;
  	$this->id = (int) $wp_post->ID;
  	$this->ID = (int) $wp_post->ID;
...
//   	$this->set_value('title', get_the_title($this->id));
//   	$this->set_value('title_plain', strip_tags(@$this->title));
//   	var_dump('json-api/models/post.php for sidebar before content_value()');
//   	var_dump(microtime());
//   	$this->set_content_value();
//   	var_dump('after content_value()');
//   	var_dump(microtime());
  	$this->set_value('excerpt', get_the_excerpt());
  	$this->set_value('date', get_the_time($date_format));
  	$this->set_value('modified', date($date_format, strtotime($wp_post->post_modified)));
