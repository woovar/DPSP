...
class JSON_API_Introspector {
	...
	public function get_menu($name = false) {
		global $menu;
		$output=wp_nav_menu(array('echo'=>false,'menu'=>'Nederlands'));
		return $output;
	}	
        public function get_menu_english($name = false) {
                global $menu;
                $output=wp_nav_menu(array('echo'=>false,'menu'=>'English'));
                return $output;
        }
	public function get_all_comments_json(){
		return commentpress_get_all_comments_content_json();
		
	}
	
	public function get_all_comments(){
		return commentpress_get_all_comments_page_content();
		
	}
	
	public function get_comments_by_commenter(){
		return commentpress_get_comments_by_page_content();
	}
	
	public function get_content_sidebar($name=false){
		
		$output=wp_get_nav_menu_items( 'DPSP Menu'); 
		var_dump($output);
		exit;
		return $output;
	}
	...
	public function get_sidebar() {
		global $sidebar,$post;
		//$menu = wp_get_nav_menu_object('DPSP Menu');
		//$output=wp_get_nav_menu_items($menu->term_id);
// 		var_dump('before introspector');
// 		var_dump(microtime());
		$output=get_sidebar();
// 		var_dump('after introspector');
// 		var_dump(microtime());
		//$menu=wp_get_nav_menu_object( 'DPSP Menu' );
  ...
  public function get_posts($query = false, $wp_posts = false) {
    ...
    #goed! 
#    $output[0]->content = $post->post_content;
    return $output;
  }
