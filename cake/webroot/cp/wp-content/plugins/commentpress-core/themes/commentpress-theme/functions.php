...
if ( ! function_exists( 'commentpress_get_all_comments_content_json' ) ):
/** 
 * @description: all-comments page display function
 * @todo: 
 *
 */
function commentpress_get_all_comments_content_json( $page_or_post = 'page' ) {
	...
	WHERE 
	$wpdb->posts.post_type = '$page_or_post' 
	AND $wpdb->comments.comment_approved = '1' 
...
function commentpress_get_comments_by_para() {
	...
	if ( count( $comments_sorted ) > 0 ) {
		...
		//var_dump(sizeof($comments_sorted));
...
// setcookie("TestCookie", $value, time()+3600, "/", "localhost", 30);
// Print an individual cookie
// var_dump($_COOKIE["TestCookie"]);
// var_dump($HTTP_COOKIE_VARS["TestCookie"]);

// 					echo "Logged in:";
// 						var_dump($_SESSION);
// 						var_dump($_COOKIE);
						
	//var_dump('wp_parse_auth_cookie');
	//var_dump($cookie);
	//var_dump('scheme ');
	//var_dump($scheme);
	//var_dump('wp_parse');
	//var_dump(wp_parse_auth_cookie($cookie, $scheme));
	if ( ! $cookie_elements = wp_parse_auth_cookie($cookie, $scheme) ) {
		do_action('auth_cookie_malformed', $cookie, $scheme);
	}
	
	extract($cookie_elements, EXTR_OVERWRITE);
	//var_dump('cookie_elements');
	//var_dump($cookie_elements);
	$expired = $expiration;
	//var_dump('expired');
	//var_dump($expired);
	...
	do_action('auth_cookie_valid', $cookie_elements, $user);
						...
						if ( get_option('comment_registration') AND !is_user_logged_in() ) {
							// leave comment link
							echo '<div class="reply_to_para" id="reply_to_para-'.$para_num.'">'."\n".
									'<p><a class="reply_to_para" rel="nofollow" href="/research/wp-login.php?redirect_to=' . $_SERVER['HTTP_REFERER'] . '">'.//JVDP
...
function commentpress_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	...
	if ( get_option('comment_registration') AND !$user_ID ) {
		...
		$link = '<a rel="nofollow" href="/research/wp-login.php?redirect_to=' . get_permalink() . '">' . $login_text . '</a>';
