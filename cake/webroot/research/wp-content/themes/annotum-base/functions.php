...
function anno_favicon() {
	echo '<link rel="shortcut icon" href="/favicon.ico" />';//JVDP
}
...
// 		wp_update_post(array('ID'=>$post_id,'excerpt'=>"asdf"));
// 	}
}
...
function modify_post_title( $data , $postarr )//JVDP to prevent connection reset error
{
	write_log($data);
	#if(empty($data['post_excerpt'])){
  	#	$data['post_excerpt'] = 'No abstract';
  	#}
  return $data;
}
...
$message = "<br/>Welcome to the Digital Paul Scholten Project, new users need to <a href='wp-login.php?action=register'>register</a> first to be able to leave comments and submit articles.<br/><br/>";
return $message;
}
add_filter('login_message', 'custom_login_message');//JVDP
...
function set_ps_cookie($user_login, $user) {
	if (!isset($_COOKIE['paulscholten_cookie'])) {
		setcookie('paulscholten_cookie', $user->display_name, time()+31536000, '/', COOKIE_DOMAIN, false);
	}
}
add_action( 'wp_login', 'set_ps_cookie',10,2); //JVDP

function remove_ps_cookie() {
	if (isset($_COOKIE['paulscholten_cookie'])) {
		setcookie('paulscholten_cookie', '', time() - 3600, '/', COOKIE_DOMAIN, false);
	}
}
add_action('wp_logout','remove_ps_cookie');//JVDP
...
function getCrunchifyPostViewsPDF($postID){
...
function setCrunchifyPostViewsPDF($postID) {
	$count_key = 'post_downloads_count';
	$count = get_post_meta($postID, $count_key, true);
	if($count==''){
		$count = 0;
