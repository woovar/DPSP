...
function anno_user_can($cap, $user_id = null, $post_id = null, $comment_id = null) {
	...
	switch ($cap) {
		...
		case 'add_general_comment':
			...
			if (($user_role && $user_role != 'reviewer')||$user_role == $admin) {
				return true;
			}
			break;
		...
		case 'add_review_comment':
			...
			if (($user_role && !in_array($user_role, array('author', 'co-author')) && $post_state == 'in_review')||$user_role == $admin) {
				return true;
			}
			break;
...
function annowf_get_role_emails($role, $post = null) {
	switch ($role) {
		case 'administrator':
			$users = get_users(array('role' => $role));//JVDP
			break;
		case 'editor':
