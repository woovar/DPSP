...
class Anno_PDF_Download {
	...
	public function setup_filterable_props() {
		...
		$this->memory_limit = apply_filters(__CLASS__.'_memory_limit', '1024M'); // only gets set when downloading the PDF
	...
	private function html4ify($html) {
		$replacements = array(
			...
			'<em' => '<div style="font-style: italic;"',
			'</em>' => '</div>',
		);
	...
	**/ 
	public static function footer_markup($page_number=null) {
		// DOMpdf markup 
		...
		<script type="text/php"> 
			if (isset($pdf)) {
			    ...
			    $pdf->page_text($x, $y, "{PAGE_NUM}'.$page_number.'", $font, $size);
