...
class CPDF_Adapter implements Canvas {
  ...
  protected function _add_page_text() {
	...
	#var_dump('HERE');
	#var_dump($text);
	if(substr($text,0,10) == '{PAGE_NUM}'){
		#$text='{PAGE_NUM}';
	
	$begin_page_number=$text;
	$begin_page_number=str_replace("{PAGE_NUM}",'', $begin_page_number);
	#var_dump($begin_page_number);
	if($begin_page_number && $page_number==1){
		$page_number=$begin_page_number;
		#var_dump($page_number);
	}
	$text=str_replace($begin_page_number,'',$text);
