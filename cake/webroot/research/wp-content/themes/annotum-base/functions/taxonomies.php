...
function anno_add_category_single_selection() {
	add_meta_box('article_category_select', _x('Name research question', 'Meta box title', 'anno'), 'anno_article_category_dropdown', 'article', 'side', 'core');
}
...
function anno_article_category_dropdown() {
	...
	if (!empty($post)) {
		...
		if (is_array($cat) && !empty($cat)) {
			...
			'name' => 'tax_input[article_category]',
			'depth' => 999,
			'show_option_all'    => 'Select a research question' //JVDP
