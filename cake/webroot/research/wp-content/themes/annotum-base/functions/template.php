...
class Anno_Template {
	 ...
	 */
	protected $enable_caches = false;//JVDP
...
#$        AND comment_type = 'article_review'
	...
	/**
	 * Get the keywords data stored as post meta //JVDP
	 */
	public function get_keywords($post_id = false) {
		$post_id = $this->utils->post_id_for_sure($post_id);
		return get_post_meta($post_id, '_anno_keywords', true);
	}
	
	/**
	 * Get the expected data stored as post meta //JVDP
	 */
	public function get_expected($post_id = false) {
		$post_id = $this->utils->post_id_for_sure($post_id);
		return get_post_meta($post_id, '_anno_expected', true);
	}	
	public function get_comments_article($post_id = false) {
		$post_id = $this->utils->post_id_for_sure($post_id);
		return get_post_meta($post_id, '_anno_comments_article', true);
	}	
	public function get_pdf_link($post_id = false) {
		$post_id = $this->utils->post_id_for_sure($post_id);
		return get_post_meta($post_id, '_anno_pdf_link', true);
	}	
	

	public function get_volume($post_id = false) {
		$post_id = $this->utils->post_id_for_sure($post_id);
		$volume = preg_replace('/.*volume_(\d)_\((\d+)\).*/','Volume \1 (\2)',get_the_term_list($post_id, 'article_tag', '', ';', ''));
		if (strpos($volume, 'Volume') === false){
			return '';
		}else{
			return $volume;
		}
	...
	public function get_page_number($post_id = false) {
		$post_id = $this->utils->post_id_for_sure($post_id);
		return get_post_meta($post_id, '_anno_page_number', true);
	}	
	/**
	...
	public function get_contributors_list($post_id = null) {
		...
		foreach ($authors as $author) {
			...
			if ($author_is_id) {
				...
				$author_data['link'] = get_the_author_meta('user_url',$author_id);
				//$author_data['bio'] = $author_wp_data->user_description;
				$author_data['bio'] = get_the_author_meta('description',$author_id);
				...
				$author_id = $author['id'];
				//var_dump(get_user_meta($author_id,'user_url'));
				...
				$author_data['bio'] = get_the_author_meta('description',$author_id);
				$author_data['link'] = get_the_author_meta('user_url',$author_id);
				$author_data['institution'] = get_the_author_meta('_anno_institution',$author_id);
				$author_data['department'] = get_the_author_meta('_anno_department',$author_id);
				$author_data['city'] = get_the_author_meta('_anno_city',$author_id);
			...
			if ($author_data['first_name'] && $author_data['last_name']) {
				//JVDP $fn = empty($posts_url) ? '<span class="name">' : '<a href="'.esc_url($posts_url).'" class="url name">';				

				$fn = $prefix_markup;

				$fn.='<span class="given-name">'.esc_html($author_data['first_name']).'</span> <span class="family-name">'.esc_html($author_data['last_name']).'</span>'.$suffix_markup;
			...
			if(!empty($author_data['link'])){
				$user_url.='<a href="'.$author_data['link'].'">';
				$user_url.='<img class="redirect-img" src="/img/arrow-redirect.png"/>';
				$user_url.= "</a>";
			}
			$institution='';
			...
			if(!empty($author_data['department'])){
				$department=$author_data['department'];
			}	
			$city='';
	...
	public function get_citation($post_id = null) {
		...
		$page_number = $this->get_page_number($post_id);
		...
		foreach ($contributors as $contributor) {
			if ($contributor_is_id) {
				...
				//$last=preg_replace('/van (.*)/i','$1, van',$last);
		...
		$edition = preg_replace('/.*edition (\d).*/','\1',get_the_term_list($post_id, 'article_tag', '', ';', ''));
		$category = preg_replace('/.*category_(\w+).*/','\1',get_the_term_list($post_id, 'article_tag', '', ';', ''));
		$volume = preg_replace('/.*volume_(\d)_\((\d+)\).*/','Volume \1 (\2)',get_the_term_list($post_id, 'article_tag', '', ';', ''));
		if ((strpos($volume, '<a href')) !== false){
			$volume='';
		}
		...
		$concept='';
		if(anno_has_expected($post_id)){
			$concept='In progress, do not cite';
		}
		if($concept){
			$citation=$concept;
		}else{
			if ($category=='research'){
				$category_str='I: Research';
			}elseif ($category=='new_translation'){
				$category_str='II: New Translations';
			}else{
				$category_str='III: Edited Reissues';
			}
			if ($volume!=''){
				if ($page_number!=''){
			$citation = sprintf(
				_x('%1$s. %2$s. DPSP Annual, %3$s, %4$s, %5$s.' , 'Citation format', 'anno'),//JVDP
				$authors, // 1
				$title, // 2
				$category_str, //3
				$volume, //3
			...
			$citation = sprintf(
				_x('%1$s. %2$s. DPSP Annual, %3$s, %4$s.' , 'Citation format', 'anno'),//JVDP
				$authors, // 1
				$title, // 2
				$category_str, //3
				$volume //3
			...
			$citation = sprintf(
				_x('%1$s. %2$s. DPSP Annual, %3$s.' , 'Citation format', 'anno'),//JVDP
				$authors, // 1
				$title, // 2
				$category_str //3
				//$volume //3
...
/**
 * Check if an article has a expected date
 */
function anno_has_expected($post_id = false) {
	$template = Anno_Keeper::retrieve('template');
	return (bool) $template->get_expected($post_id);
}
function anno_has_comments_article($post_id = false) {
	$template = Anno_Keeper::retrieve('template');
	return (bool) $template->get_comments_article($post_id);
}

function anno_has_pdf_link($post_id = false) {
	$template = Anno_Keeper::retrieve('template');
	return (bool) $template->get_pdf_link($post_id);
}

function anno_has_page_number($post_id = false) {
	$template = Anno_Keeper::retrieve('template');
	return (bool) $template->get_page_number($post_id);
}
/**
 * Check if an article has a subtitle
 */
function anno_has_keywords($post_id = false) {
	$template = Anno_Keeper::retrieve('template');
	return (bool) $template->get_keywords($post_id);
}
...
/**
 * Output expected data stored as post meta
 */
function anno_the_expected() {
	$template = Anno_Keeper::retrieve('template');
	echo $template->get_expected();
}
function anno_the_comments_article() {
	$template = Anno_Keeper::retrieve('template');
	echo $template->get_comments_article();
}
function anno_the_pdf_link() {
	$template = Anno_Keeper::retrieve('template');
	echo $template->get_pdf_link();
}

function anno_the_comments() {
	$template = Anno_Keeper::retrieve('template');
	return $template->get_comments();
}

function anno_the_volume() {
        $template = Anno_Keeper::retrieve('template');
        return $template->get_volume();
}
/**
 * Output subtitle data stored as post meta
 */
function anno_the_keywords() {
	$template = Anno_Keeper::retrieve('template');
...
function anno_the_authors() {
	$template = Anno_Keeper::retrieve('template');
	return $template->get_contributors_list();
}
