...
function anno_article_meta_boxes($article) {
	add_meta_box('subtitle', _x('Subtitle', 'Meta box title', 'anno'), 'anno_subtitle_meta_box', 'article', 'normal', 'high'); //JVDP
	add_meta_box('expected', _x('When can the full article be expected? (leave empty when not appropriate)', 'Meta box title', 'anno'), 'anno_expected_meta_box', 'article', 'normal', 'high'); //JVDP
	add_meta_box('pdf_link', _x('PDF link (leave empty to generate)', 'Meta box title', 'anno'), 'anno_pdf_link_meta_box', 'article', 'normal', 'high'); //JVDP
	add_meta_box('comments_article', _x('Comments', 'Meta box title', 'anno'), 'anno_comments_article_meta_box', 'article', 'normal', 'high'); //JVDP
	add_meta_box('page_number', _x('Page numbers', 'Meta box title', 'anno'), 'anno_page_number_meta_box', 'article', 'normal', 'high'); //JVDP
	add_meta_box('abstract', _x('Abstract', 'Meta box title', 'anno'), 'anno_abstract_meta_box', 'article', 'normal', 'high');
	add_meta_box('keywords', _x('Keywords', 'Meta box title', 'anno'), 'anno_keywords_meta_box', 'article', 'normal', 'high'); //JVDP
	add_meta_box('abstract', _x('Abstract', 'Meta box title', 'anno'), 'anno_abstract_meta_box', 'article', 'normal', 'high');
	add_meta_box('keywords', _x('Keywords', 'Meta box title', 'anno'), 'anno_keywords_meta_box', 'article', 'normal', 'high'); //JVDP
	add_meta_box('body', _x('Body', 'Meta box title', 'anno'), 'anno_body_meta_box', 'article', 'normal', 'high');
	//JVDP add_meta_box('references', _x('References', 'Meta box title', 'anno'), 'anno_references_meta_box', 'article', 'normal', 'high');
	//JVDP add_meta_box('funding', _x('Funding Statement', 'Meta box title', 'anno'), 'anno_funding_meta_box', 'article', 'normal', 'high');
	add_meta_box('acknowledgements', _x('Acknowledgements', 'Meta box title', 'anno'), 'anno_acknowledgements_meta_box', 'article', 'normal', 'high');
	//JVDP add_meta_box('appendices', _x('Appendices', 'Meta box title', 'anno'), 'anno_appendices_meta_box', 'article', 'normal', 'high');
	//JVDP add_meta_box('featured', _x('Featured', 'Meta box title', 'anno'), 'anno_featured_meta_box', 'article', 'side', 'default');

	if (current_user_can('editor') || current_user_can('administrator')) {
		//JVDP add_meta_box('convert', _x('Convert To Post', 'Meta box title', 'anno'), 'anno_convert_meta_box', 'article', 'side', 'low');
...
function anno_pdf_link_meta_box($post) {
	$html = get_post_meta($post->ID, '_anno_pdf_link', true);
	?>
	<input type="text" name="anno_pdf_link" value="<?php echo esc_attr($html); ?>" style="width:100%;" />
<?php
}
function anno_expected_meta_box($post) {
	$html = get_post_meta($post->ID, '_anno_expected', true);
	?>
	<input type="text" name="anno_expected" value="<?php echo esc_attr($html); ?>" style="width:100%;" />
<?php
}

function anno_comments_article_meta_box($post) {
	$html = get_post_meta($post->ID, '_anno_comments_article', true);
	?>
	<input type="text" name="anno_comments_article" value="<?php echo esc_attr($html); ?>" style="width:100%;" />
<?php
}

function anno_page_number_meta_box($post) {
	$html = get_post_meta($post->ID, '_anno_page_number', true);
	?>
	<input type="text" name="anno_page_number" value="<?php echo esc_attr($html); ?>" style="width:100%;" />
<?php
}

function anno_keywords_meta_box($post) {
	$html = get_post_meta($post->ID, '_anno_keywords', true);
	?>
	<input type="text" name="anno_keywords" value="<?php echo esc_attr($html); ?>" style="width:100%;" />
<?php
}
...
function anno_article_save_post($post_id, $post) {
	if ($post->post_type == 'article') {
		$anno_meta = array(
			...
			'anno_expected',//JVDP
			'anno_pdf_link',//JVDP
			'anno_comments_article',
			'anno_page_number',//JVDP
			'anno_funding',
		...
		foreach ($anno_meta as $key) {
			if (isset($_POST[$key])) {
				switch ($key) {			
					...
					case 'anno_expected':
					case 'anno_pdf_link':
					case 'anno_comments_article':
					case 'anno_page_number':
					case 'anno_funding':
